...
class GeneralStore<T extends EntityWithId> {
  ...
  constructor(storeName: string, defaultData: T[]) {
    ...
    if (typeof window !== 'undefined') {
      const localData = JSON.parse(localStorage.getItem(storeName) || '[]' , dateReviver)
      const mergedData = localData.concat(defaultData.filter((item: T) => !localData.some((localItem : T) => localItem.id === item.id)))
  ...
  private getLatestId() {
    ...
    const latestId = Math.max(...this.data.map(item => parseInt(item.id, 10)), -1) + 1

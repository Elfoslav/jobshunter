...
const getSimilarJobs = async (
  ...
  let filteredJobs = jobs.filter((_job) => {
    ...
    const hasCommonRequiredSkill = _job.requiredSkills.some((requiredSkills) =>
      job.requiredSkills.includes(requiredSkills)
    );
    const hasCommonOptionalSkill = _job.optionalSkills.some((optionalSkills) =>
      job.optionalSkills.includes(optionalSkills)
    );
...
export const useGetJobs = (
  ...
  const result = useQuery<ExistingJob[], unknown>(
    [JOBS_QUERIES.JOBS, page, searchQuery, skills],
    async () => {
...
export const useGetJobsCount = (
  ...
  const result = useQuery<number, unknown>(
    [JOBS_QUERIES.JOBS_COUNT, searchQuery, skills],
    () => getJobsCount(searchQuery, skills)
  );
  return { ...result };
};
...
export const useGetJobById = (jobId: string) => {
  const result = useQuery<ExistingJob | null, unknown>(
    [JOBS_QUERIES.JOB_BY_ID, jobId],
    () => getJobById(jobId)
  );
  return result;
};
